...
Vue.component('cart-item', {
    ...
    methods: {
        countDown(item) {
            ...
            if (item.count <= 0) {
                let itemToDelete = app.cartItems.indexOf(item);
                app.cartItems.splice(itemToDelete, 1);
    ...
    methods: {
        addToCart(item) {
            ...
            if (app.cartItems.length > 0) {
                for (let i = 0; i < app.cartItems.length; i++) {
                    if (item.id_product == app.cartItems[i].id_product) {
                        app.cartItems[i].count++;
                        isNew = false;
                    }
                }
            }
            if (isNew) {
                app.cartItems.push({ ...item, 'count': 1 });
            }
            localStorage.setItem('cartItems', JSON.stringify(app.cartItems));
        },
    }
